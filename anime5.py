...
# Achievements
USER_ACHIEVEMENTS = {}  # {user_id: [achievement_names]}
...
DEFAULT_CHAR_IMG = "https://files.catbox.moe/knsy3i.jpg"
...
async def profile_cmd(c, m):
    ...
    achievements_txt = "\n".join([f"  {achi}" for achi in achievements]) if achievements else "  No achievements yet"
...
async def add_char_cmd(c, m):
    ...
    try:
        ...
        if not is_admin(m.from_user.id):
            return await m.reply("❌ Only admins can use this command!")
...
@app.on_message(filters.command("updateimg"))
    ...
    try:
        if os.path.exists(ACHIEVEMENTS_FILE):
            with open(ACHIEVEMENTS_FILE, 'r', encoding='utf-8') as f:
                ...
                logging.info(f"Loaded achievements for {len(USER_ACHIEVEMENTS)} users.")
        else:
            USER_ACHIEVEMENTS = {}
    except Exception as e:
...
def save_achievements():
    ...
    try:
        ...
        logging.info(f"Saved achievements for {len(USER_ACHIEVEMENTS)} users.")
    except Exception as e:
...
def add_achievement(user_id, achievement_key):
    ...
    if achievement_key not in USER_ACHIEVEMENTS[user_id_str]:
        USER_ACHIEVEMENTS[user_id_str].append(achievement_key)
        save_achievements()
        return True
    return False

def remove_achievement(user_id, achievement_key):
    """Remove an achievement from a user."""
    user_id_str = str(user_id)
    if user_id_str in USER_ACHIEVEMENTS:
        if achievement_key in USER_ACHIEVEMENTS[user_id_str]:
            USER_ACHIEVEMENTS[user_id_str].remove(achievement_key)
            save_achievements()
            return True
    return False
...
def get_user_achievements(user_id):
    ...
    user_id_str = str(user_id)
    achi_keys = USER_ACHIEVEMENTS.get(user_id_str, [])
    return [AVAILABLE_ACHIEVEMENTS.get(k, k) for k in achi_keys if k in AVAILABLE_ACHIEVEMENTS]
...
@app.on_message(filters.command("addachi"))
async def addachi_cmd(c, m):
    """
    Admin command to add an achievement to a user.
    Usage:
    /addachi <achievement_key>
    (Reply to a message from the user)
    
    OR
    
    /addachi <achievement_key> @username
    /addachi <achievement_key> 123456789
    """
    try:
        if not is_admin(m.from_user.id):
            return await m.reply("❌ Only admins can use this command!")
        ...
        if len(parts) < 2:
            available = ", ".join(AVAILABLE_ACHIEVEMENTS.keys())
            return await m.reply(
                f"❌ Invalid format.\n\n"
                f"<b>Reply to a message:</b>\n/addachi <achievement_key>\n\n"
                f"<b>Or provide UID:</b>\n/addachi <achievement_key> 123456789\n\n"
                f"<b>Or provide username:</b>\n/addachi <achievement_key> @username\n\n"
                f"Available achievements:\n{available}",
                parse_mode=ParseMode.HTML
            )
        
        achievement_key = parts[1].strip()
        
        if achievement_key not in AVAILABLE_ACHIEVEMENTS:
            available = ", ".join(AVAILABLE_ACHIEVEMENTS.keys())
            return await m.reply(
        ...
        success = add_achievement(target_uid, achievement_key)
        if success:
            achi_name = AVAILABLE_ACHIEVEMENTS[achievement_key]
            await m.reply(f"✅ Added achievement {achi_name} to user {target_uid}")
        else:
            await m.reply(f"⚠️ User {target_uid} already has this achievement!")
    
    except Exception as e:
        logging.exception(f"Error in addachi_cmd: {e}")
        await m.reply(f"❌ Error: {e}")

@app.on_message(filters.command("rmachi"))
async def rmachi_cmd(c, m):
    """
    Admin command to remove an achievement from a user.
    Usage:
    /rmachi <achievement_key>
    (Reply to a message from the user)
    
    OR
    
    /rmachi <achievement_key> @username
    /rmachi <achievement_key> 123456789
    """
    try:
        if not is_admin(m.from_user.id):
            return await m.reply("❌ Only admins can use this command!")
        ...
        success = remove_achievement(target_uid, achievement_key)
        if success:
            achi_name = AVAILABLE_ACHIEVEMENTS[achievement_key]
            await m.reply(f"✅ Removed achievement {achi_name} from user {target_uid}")
        else:
            await m.reply(f"⚠️ User {target_uid} doesn't have this achievement!")
    
    except Exception as e:
        logging.exception(f"Error in rmachi_cmd: {e}")
        await m.reply(f"❌ Error: {e}")

@app.on_message(filters.command("addpoints"))
async def addpoints_cmd(c, m):
    """Admin command to add ELO points to a user."""
    try:
        if not is_admin(m.from_user.id):
            return await m.reply("❌ Only admins can use this command!")
        ...
        try:
            ...
            return await m.reply("❌ User ID and points must be integers!")
        
        # Get current stats
        stats = get_user_stats(user_id, "Unknown")
        old_rating = stats.get("rating", 1200)
        new_rating = max(0, old_rating + points)
        ...
        logging.exception(f"Error in addpoints_cmd: {e}")
        await m.reply(f"❌ Error: {e}")

@app.on_message(filters.command("removepoints"))
async def removepoints_cmd(c, m):
    """Admin command to remove ELO points from a user."""
    try:
        if not is_admin(m.from_user.id):
            return await m.reply("❌ Only admins can use this command!")
        ...
        logging.exception(f"Error in removepoints_cmd: {e}")
        await m.reply(f"❌ Error: {e}")
