...
def _forfeit_game_actions(chat_id, gid, game):
    ...
    try:
        ...
        # Forfeit message removed to reduce spam in group chats
    except Exception:
...
def update_leaderboard_elo(p1_id, p1_name, p2_id, p2_name, result):
    ...
    if result == 0.5:
        ...
        if result == 1:
            ...
            # More than 1 rank below winner
            if diff > 1:
                delta_w, delta_l = 16, -8
            # 1 rank below winner
            elif diff == 1:
                delta_w, delta_l = 21, -10  # Average of 17-25 range
            # Same rank
            elif diff == 0:
                delta_w, delta_l = 27, -13  # Average of 25-30 range
            # Winner defeated someone one rank higher
            elif diff == -1:
                delta_w, delta_l = 35, -15
            else:
                # More than 1 rank higher than loser
                delta_w, delta_l = 35, -10
            ...
            # 1 rank below winner
                ...
                delta_w, delta_l = 21, -10
            # Same rank
            elif diff == 0:
                delta_w, delta_l = 27, -13
            # Winner defeated someone one rank higher
            elif diff == -1:
                delta_w, delta_l = 35, -15
            else:
        ...
        try:
            ...
            if data.startswith("lb_toggle_"):
                ...
                try:
                    ...
                    pass # If edit fails, do nothing
                await q.answer()
                return

            if chat_id not in GAMES:
                # If game data is completely missing for the chat, alert user and try to cleanup UI
                await q.answer("❌ Game expired.", show_alert=True)
                try:
                    await q.message.edit_text("❌ Game expired.", reply_markup=None)
                except Exception:
                    pass # Do NOT send a new message if edit fails
                return
            ...
            # --- FIX STARTS HERE ---
            if not game:
                # 1. Show an alert to the user so they know why it didn't work
                await q.answer("❌ Game expired.", show_alert=True)
                
                # 2. Try to edit the message to remove buttons/show expired text
                try:
                    await q.message.edit_text("❌ Game expired.", reply_markup=None)
                except FloodWait as e:
                    # If floodwait, wait and try one last time
                    await asyncio.sleep(e.value)
                    try:
                        await q.message.edit_text("❌ Game expired.", reply_markup=None)
                    except Exception:
                    ...
                    # If message is deleted, too old, or cannot be edited:
                    # We pass here. We explicitly do NOT send a new message.
                    pass
                return
            # --- FIX ENDS HERE ---
