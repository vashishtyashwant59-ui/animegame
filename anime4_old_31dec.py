...
def _forfeit_game_actions(chat_id, gid, game):
    ...
    try:
        ...
        # Try to notify chat (schedule on app loop)
        try:
            if hasattr(app, 'loop') and app.loop is not None:
                # Ensure the coroutine is created on the app loop thread.
                app.loop.call_soon_threadsafe(lambda: asyncio.create_task(
                    app.send_message(chat_id, note, parse_mode=ParseMode.HTML)
                ))
        except Exception:
            logging.exception('Failed to send forfeit notification')
    except Exception:
...
def update_leaderboard_elo(p1_id, p1_name, p2_id, p2_name, result):
    ...
    new_r1 = calculate_elo(r1, r2, result)
    new_r2 = calculate_elo(r2, r1, 1 - result) # If p1 gets 1, p2 gets 0
    ...
    return (new_r1 - r1), (new_r2 - r2) # Return change
...
async def finish_game_ui(client, message, game, game_id):
    ...
    kb = [
        [InlineKeyboardButton(f"🔵 {p1n} READY", callback_data=f"start_rpg_p1_{game_id}")],
        [InlineKeyboardButton(f"🔴 {p2n} READY", callback_data=f"start_rpg_p2_{game_id}")]
    ]
    await ensure_display_message(client, message.chat.id, game, text, reply_markup=kb, preview=False)
...
async def simulate_battle(client, message, game):
    ...
    for r1, r2, title, pts in matchups:
        ...
        if c1 and c2:
            ...
            if r1 == "Assassin" and r2 == "Healer": s1 = int(s1 * 1.5)
            if r1 == "Healer" and r2 == "Assassin": s2 = int(s2 * 1.5)
    ...
    if t1:
        if random.randint(0, 100) > get_stat(t1, "Traitor"):
            score1 -= 30
    ...
    if t2:
        if random.randint(0, 100) > get_stat(t2, "Traitor"):
            score2 -= 30
